...
//GLOBAL VARIABLE
// WiFi parameters
const char* ssid = "Adim";
const char* password = "Adim0101";

#define ONE_WIRE_BUS D3 //pin for temperature sensor

boolean noErrors = true;
int NodeNumber = 4;
int heart_beat_value;
int myBPM = 0;
...
void loop() {
  if (noErrors) {
    ...
    PostingData(tempC, myBPM);
    myBPM = heart_beat_handler();
  }
  else {
...
void PostingData(float tempC, int myBPM)                        //Score on screen
{
  ...
  String serverName = host + "/" + nodeNum + "/" + tempC + "/" + myBPM;
...
int heart_beat_handler() {
  heart_beat_value = random(81, 89);
...
void clearLCDLine(int row, int col) {
  ...
  delay(250);// set cursor in the beginning of deleted line
}
//--------------------------------------------------------------------------------
//===============================================================================================
